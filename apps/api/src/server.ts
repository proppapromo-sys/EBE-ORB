...
// Pull any owner-set keys (set from the in-app Keys panel) into memory before serving requests.
void loadPlatformKeys();

const app = express();
app.use(cors());

// The entire ORB platform, under /api/orb — the same call any other build uses.
mountOrb(app);
