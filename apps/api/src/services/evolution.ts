 ...
 */
export const EVOLVE_QUERY = /\b(what should (?:i|we|orb|you) become|who (?:do (?:i|we) want to|should (?:i|we|you)) become|next version of (?:myself|me|us|orb|you)|evolve (?:into|toward|past|beyond)?|conscious(?:ly)? evolv|reinvent (?:myself|ourselves|the)|grow into|next (?:stage|phase|chapter|level) of|who (?:am i|are we) becoming|where (?:should|do) (?:i|we) (?:go|grow) (?:next|from here)|evolution gap|future[- ]ready|prepare (?:myself|us|for what'?s next)|what (?:capabilities|skills) (?:do (?:i|we) need|should (?:i|we) build) (?:next|for the future)|redesign (?:myself|how (?:i|we)|(?:our|its|my) own (?:architecture|approach))|what (?:must|do) (?:i|we) become|surpass (?:itself|myself|ourselves|our (?:current )?(?:self|form))|how does? (?:a |the )?system surpass itself|outgrow (?:our|its|the|my) (?:architecture|structure|current)|becomes? (?:its|their|the) own (?:limit|limitation|constraint)|what (?:architecture|structure|assumptions?) must (?:we|i) outgrow|endless (?:transformation|becoming|change)|continuous(?:ly)? becoming|navigate (?:endless|constant|continuous) (?:transformation|change|becoming)|always (?:be )?(?:transforming|becoming|evolving)|infinite becoming|state of becoming)\b/i;
