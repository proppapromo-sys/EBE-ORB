 ...
 */
export const SOURCE_QUERY = /\b(where (?:does|did) (?:this|it|that)\b.{0,30}?come from|trace (?:it|this|that) back|trace (?:the|its) (?:origin|source|roots?)|the (?:deeper|deepest|underlying|ultimate|original) (?:cause|source|origin|reason|driver)|what (?:created|generated|caused|drives?|is driving|is generating) (?:this|it|that|the)|what'?s (?:underneath|beneath|behind) (?:this|it|that|the)|symptom(?:s)? (?:vs|versus|not the) (?:cause|root|source)|go deeper than|five whys|5 whys|keep asking why|what'?s the (?:source|origin) of|originate(?:s|d)? from|what conditions (?:created|led to|gave rise)|get past the symptom|address the (?:root|source) (?:not|rather than)|source from which (?:everything|all|it all)|deepest (?:foundation|source|layer|root)|what (?:generates|is the source of) (?:everything|all|reality|existence)|from which (?:everything|all things?) emerge|fewest assumptions|first cause|foundational (?:cause|principle|condition))\b/i;
