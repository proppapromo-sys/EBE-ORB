...
export const STRATEGIC = /\bshould (?:i|we)\b[^?]*\b(launch|expand|hire|fire|lay off|invest|raise|buy|sell|acquire|partner|quit|leave|relocate|open|shut|close|merge|pivot|borrow|sign|commit|go all in|double down|take on)\b|\b(big|major|life|huge|hard) (?:decision|move|bet|call)\b|\bbet the (?:house|company|farm)\b|\bis (?:it|this) (?:wise|the right (?:thing|move|call)|worth it|the wise (?:move|call|thing))\b|\bwhat(?:'s| is) the wisest (?:thing|course|move|option|choice)\b|\bwisest thing to do\b|\bsecond[- ]?order (?:effect|consequence)|\bthird[- ]?order (?:effect|consequence)|\bgreatest long[- ]?term good\b|\bgiven everything\b[^?]*\bwhat (?:should|do) (?:i|we)\b/i;
