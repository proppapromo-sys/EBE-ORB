 ...
 */
export const ANTIFRAGILE_QUERY = /\b(antifragil(?:e|ity)|stronger (?:from|through|because of)|gain from (?:disorder|stress|chaos|volatility)|what (?:if|happens (?:if|when)) .{0,40}?(?:fails?|drops?|breaks?|goes wrong|collapses?)|single point of failure|concentration risk|dependency risk|what could break|stress[- ]?test|fail[- ]?safe|redundan(?:t|cy)|fallback|contingency|worst[- ]?case|too dependent on|over[- ]?reliant|optionality|keep (?:my|our) options open|hedge|spread the risk|resilien(?:t|ce) (?:to|against)|bounce back|recover (?:from|faster)|learn from (?:this )?failure|turn (?:this|it|adversity|the setback) (?:into|to) (?:an? )?(?:advantage|strength)|setback|which decisions? (?:can(?:'?t| ?not)|cannot) be (?:undone|reversed)|what(?:'s| is)? irreversible|irreversible (?:decision|choice|harm|loss)|(?:expand|reduce|close|open) (?:future |our )?(?:options|doors|possibilities|optionality)|expand or reduce.{0,20}?options|close (?:off )?(?:doors|options|future)|keep (?:the )?(?:doors?|future|options?) open|preserve (?:future )?optionalit|reversible (?:vs|versus|or) irreversible|doors? (?:future|that).{0,30}?(?:need|open))\b/i;
