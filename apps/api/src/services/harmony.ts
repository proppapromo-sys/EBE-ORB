 ...
 */
export const HARMONY_QUERY = /\b(balance|imbalance|out of balance|work[-/ ]?life|sustainab(?:le|ility)|burn(?:ing)? ?out|burnt out|overextend|spread (?:too )?thin|trade[- ]?off|tradeoffs?|at the (?:expense|cost) of|competing (?:priorities|demands|forces)|too much (?:on|focus on)|juggl(?:e|ing)|equilibrium|harmon(?:y|ious|ize)|stretched thin|stability vs|growth vs|sustainable pace|keep (?:it )?(?:all )?balanced|spread myself)\b/i;
