...
export async function sendMail(to: string, subject: string, text: string, replyTo?: string): Promise<MailResult> {
  ...
  const reply = replyTo || process.env.EBE_REPLY_TO; // replies go to the owner's inbox
  try {
