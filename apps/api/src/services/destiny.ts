 ...
 */
export const DESTINY_QUERY = /\b(trajectory|where (?:is|are) (?:this|it|we|things|i) (?:heading|headed|going|trending)|where (?:will|would) (?:i|we|this) (?:be|end up) (?:in|if)|if (?:i|we) (?:continue|keep|keep going|stay)\b.{0,18}?(?:path|course|trajectory|way|road|rate|direction)|where (?:does|will) (?:this|that|it) (?:path|road|course) lead|direction (?:of travel|we'?re (?:heading|going))|momentum|path[- ]?depend|where (?:we'?re|this is) headed|what (?:future|destiny) (?:is|are we) (?:emerging|forming|creating|heading toward)|the way (?:things are|i'?m) (?:going|trending)|extrapolate|at this rate|if (?:current )?(?:trends?|forces?) continue|5 years.{0,10}?(?:this (?:path|rate)|continue)|long[- ]?term trajectory)\b/i;
